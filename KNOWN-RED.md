...
commit). Re-run the 16-seed sweep before tightening.
...
bid × the per-cell factor at the deepest divergent cell. Measured at the
per-cluster-memory commit (`calibsweep --seeds 4`, seeds 0–3 + 9, 13):
6/6 green, 13–25 deep / 14–21 thin divergent cells per seed, call-site rel
err 0 exactly. Source mutants both run and reverted at the same commit:
citywide-only restored reds legs 1 and 3 on 6/6 (convergence 0/14..0/27,
factor ≡ use factor at the call site), zero shrinkage prior reds exactly
leg 2 on 6/6 (shrinkage 0/14..0/19). n0 = 2 picked from the measured
per-cell completion depths on the same runs (min 1, p25 1–2, median 2–3,
p90 4–5, max 6–8 over 71–86 cells; EconParams.CalibClusterShrinkN0).
