...
**AT THE T3 INVESTIGATION COMMIT** the suite is still **51 checks** — nothing
was added or deleted; two existing checks were restated on measurement and the
model was not touched at all. The whole diff is the harness plus two parameter
comments plus three read-only accessors on `LaborAuction`, so `fingerprint
--check` matches on all 13 lanes with no accept, and the canaries are expected
unchanged for the same reason. New instruments, each of which is what its
item's claims were measured with: `clearsweep` (the clearing-price check alone
across seeds — 1.9 s a seed against 434 s for the suite, and the first sweep
this check has ever had), `collapseprobe`, `weberprobe`, `laborprobe`, and the
mutant switch `--mutant-spare-probed-cluster`. The rows below carry the
per-item numbers.
