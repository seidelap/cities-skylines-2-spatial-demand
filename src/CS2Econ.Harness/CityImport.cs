...
namespace CS2Econ.Harness
{
    ...
    public static class CityImport
    {
        ...
        public static (WorldState w, ImportedAccess access) BuildWorld(string path, Options cfg, EconParams p)
        {
            ...
            // Resource sites: the grid template's corner patches become the
            // clusters nearest two opposite bounding-box corners (~2% each).
            float minX = float.MaxValue, minY = float.MaxValue, maxX = float.MinValue, maxY = float.MinValue;
            ...
            var resourceSite = new bool[C];
            int perCorner = Math.Max(2, C / 50);
            foreach (var (px, py) in new[] { ((double)minX, (double)minY), ((double)maxX, (double)maxY) })
            {
                var byDist = new List<(double d2, int c)>();
                for (int c = 0; c < C; c++)
                {
                    double dx = access.ClusterX[c] - px, dy = access.ClusterY[c] - py;
                    byDist.Add((dx * dx + dy * dy, c));
                }
                byDist.Sort((a, b) => a.d2 != b.d2 ? a.d2.CompareTo(b.d2) : a.c.CompareTo(b.c));
                for (int i = 0; i < perCorner; i++) resourceSite[byDist[i].c] = true;
            }

            // ---- zoning template by radius (SyntheticCity thresholds) --------
            ZoneKind ZoneFor(int c, ref SplitMix64 rng)
            {
                double r = radius[c] / maxR;
                double roll = rng.NextDouble();
                if (resourceSite[c]) return roll < 0.6 ? ZoneKind.Extractor : ZoneKind.Industrial;
            ...
            double firmMoney = Seeding.SeedFirms(w);
            ...
            foreach (var res in new[] { Res.Raw, Res.Goods })
            {
                ...
                AddExit(ExitMode.Road, exitA, res, anchor, t: 0.62, d: 2);
                AddExit(ExitMode.Road, exitB, res, anchor, t: 0.62, d: 2);
                if (cfg.RailTerminal)
                    AddExit(ExitMode.Rail, exitB, res, anchor, t: 0.10, d: 1, handling: 0.50);
                if (cfg.SeaExit)
