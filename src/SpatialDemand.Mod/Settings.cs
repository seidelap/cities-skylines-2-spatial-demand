...
namespace SpatialDemand.Mod
{
    ...
    public sealed class Locale : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts) => new Dictionary<string, string>
        {
            ...
            [settings.GetOptionLabelLocaleID(nameof(Settings.ApplyNegotiatedWages))] = "Apply negotiated retention wages (unavailable)",
            [settings.GetOptionDescLocaleID(nameof(Settings.ApplyNegotiatedWages))] = "Temporarily unavailable after an in-game pathfinding crash. This option has no effect: payroll hooks are not installed, salaries remain vanilla and saved agreements stay dormant. Hypothetical labor evaluation remains available.",
